...
drone1_trace = deque(maxlen=200)  # Stores the past positions (20 seconds * 10 Hz = 200 samples)
drone2_trace = deque(maxlen=200)
drone3_trace = deque(maxlen=200)
drone4_trace = deque(maxlen=200)
drone5_trace = deque(maxlen=200)
...
def plot_drones():
    ...
    while not rospy.is_shutdown():
        ...
        if drone1_position:
            ...
            # Plot past traces
            for i, (p1, p2, p3, p4, p5) in enumerate(
                    zip(drone1_trace, drone2_trace, drone3_trace, drone4_trace, drone5_trace)):
            ...
            drone1_arrow_dx = np.cos(drone1_yaw)
            drone1_arrow_dy = np.sin(drone1_yaw)
            drone2_arrow_dx = np.cos(drone2_yaw)
            drone2_arrow_dy = np.sin(drone2_yaw)
            drone3_arrow_dx = np.cos(drone3_yaw)
            drone3_arrow_dy = np.sin(drone3_yaw)
            drone4_arrow_dx = np.cos(drone4_yaw)
            drone4_arrow_dy = np.sin(drone4_yaw)
            drone5_arrow_dx = np.cos(drone5_yaw)
            drone5_arrow_dy = np.sin(drone5_yaw)
