...
#And each drones' VC locations are plotted (in local coordinates)
...
import math
from iq_gnc.msg import VL_local, virtual_centre
...
vc1_trace = deque(maxlen=50)
vc2_trace = deque(maxlen=50)
vc3_trace = deque(maxlen=50)
vc4_trace = deque(maxlen=50)
vc5_trace = deque(maxlen=50)
...
def drone1_vc_callback(msg):
    global drone1_vc
    drone1_vc=msg.position

def drone2_vc_callback(msg):
    global drone2_vc
    drone2_vc=msg.position

def drone3_vc_callback(msg):
    global drone3_vc
    drone3_vc=msg.position

def drone4_vc_callback(msg):
    global drone4_vc
    drone4_vc=msg.position

def drone5_vc_callback(msg):
    global drone5_vc
    drone5_vc=msg.position
...
def plot_drones():
    ...
    while not rospy.is_shutdown():
        ...
        if drone1_position:
            ...
            ax.set_ylim(-50, 50)
            ax.set_aspect('equal', adjustable='box')
            ax.set_xticks(np.arange(-50, 300, 20))
            ...
            if current_time - last_time > 0.1:  # Store a new position every 0.1 seconds (10 Hz)
                ...
                vl_trace.append(pr)
                vc1_trace.append(drone1_vc)
                vc2_trace.append(drone2_vc)
                vc3_trace.append(drone3_vc)
                vc4_trace.append(drone4_vc)
                vc5_trace.append(drone5_vc)
                last_time = current_time

            # Plot past traces
            for i, (p1, p2, p3, p4, p5, vl, vc1, vc2, vc3, vc4, vc5) in enumerate(
                    zip(drone1_trace, drone2_trace, drone3_trace, drone4_trace, drone5_trace, vl_trace, vc1_trace, vc2_trace, vc3_trace, vc4_trace, vc5_trace)):
                ...
                ax.scatter(vl.x, vl.y, c=[[0, 1, 1, alpha]], s=5)  # VL in cyan

                ax.scatter(vc1.x, vc1.y, c=[[0, 0, 1, alpha]], s=5)  # Drone 1 in blue
                ax.scatter(vc2.x, vc2.y, c=[[1, 0, 0, alpha]], s=5)  # Drone 2 in red
                ax.scatter(vc3.x, vc3.y, c=[[0, 1, 0, alpha]], s=5)  # Drone 3 in green
                ax.scatter(vc4.x, vc4.y, c=[[1, 1, 0, alpha]], s=5)  # Drone 4 in yellow
                ax.scatter(vc5.x, vc5.y, c=[[1, 0, 1, alpha]], s=5)  # Drone 5 in magenta
            ...
            ax.scatter(drone1_vc.x, drone1_vc.y, c='blue', label='VC 1', alpha=0.5)
            ax.scatter(drone2_vc.x, drone2_vc.y, c='red', label='VC 2', alpha=0.5)
            ax.scatter(drone3_vc.x, drone3_vc.y, c='green', label='VC 3', alpha=0.5)
            ax.scatter(drone4_vc.x, drone4_vc.y, c='yellow', label='VC 4', alpha=0.5)
            ax.scatter(drone5_vc.x, drone5_vc.y, c='magenta', label='VC 5', alpha=0.5)
...
def main():
    ...
    rospy.Subscriber('fury_1/virtual_centre', virtual_centre, drone1_vc_callback)
    rospy.Subscriber('fury_2/virtual_centre', virtual_centre, drone2_vc_callback)
    rospy.Subscriber('fury_3/virtual_centre', virtual_centre, drone3_vc_callback)
    rospy.Subscriber('fury_4/virtual_centre', virtual_centre, drone4_vc_callback)
    rospy.Subscriber('fury_5/virtual_centre', virtual_centre, drone5_vc_callback)
