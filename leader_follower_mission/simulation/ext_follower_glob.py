...
class DroneController:
    def __init__(self):
        ...
        rospy.Subscriber('/fury_1/drone_states',DroneStates,self.leader_states_callback)
    ...
    def control_loop(self):
        ...
        follow_distance = rospy.get_param(f"{self.drone_name}extrapol/follow_distance")  # 1 meter behind
        left_distance = rospy.get_param(f"{self.drone_name}extrapol/left_distance")  # 1 meter to the left
        alt_distance = rospy.get_param(f"{self.drone_name}extrapol/alt_distance") # 1 metre above
        ...
        while not rospy.is_shutdown():
            ...
            setpoint_msg.type_mask = GlobalPositionTarget.IGNORE_AFX + GlobalPositionTarget.IGNORE_AFY + GlobalPositionTarget.IGNORE_AFZ + GlobalPositionTarget.IGNORE_YAW_RATE #+ GlobalPositionTarget.IGNORE_VX + GlobalPositionTarget.IGNORE_VY + GlobalPositionTarget.IGNORE_VZ
            ...
            if(math.sqrt(self.vel_actual.x**2+self.vel_actual.y**2)>10):
                global horz_vel_exceed
                horz_vel_exceed+=1
                print("vel-limit exceeded")

            self.rate.sleep()

        print('Exceeded vf'+str(droneid)+':',horz_vel_exceed)
...
if __name__ == '__main__':
    try:
        ...
        print('Exceeded vf'+str(controller.droneid)+':',horz_vel_exceed)
        pass